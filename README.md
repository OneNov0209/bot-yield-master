...
**BOT Yield Master** is a decentralized application (dApp) built on the **BOT Chain Mainnet**. It combines **AI-driven automation** with **DeFi yield strategies** to help users optimize their crypto asset management. Users can deposit, withdraw, and monitor the performance of multiple AI Agents in real-time.
...
- **Deposit & Withdraw**  
  Users can deposit BOT into an Agent, see their status change to "Active", and receive estimated ROI projections. Withdrawal is executed via on-chain `withdraw(shares)` function.
...
The project runs on the **BOT Chain Mainnet**.
...
- **AutoYieldVault** (Solidity 0.8.20) — deployed on BOT Chain Mainnet
- Accepts **native BOT** directly (via `receive()`)
...
- When a user deposits `X BOT`, the contract calculates shares received:
  ```
...
- **`receive()`**: Accepts native BOT sent directly to the contract (via normal transfer).
...
**Example:**  
- User deposits 100 BOT → total funds = 100 BOT.
- Vault generates 10 BOT profit → total funds = 110 BOT.
- User withdraws all (110 BOT):
  - Performance Fee = 10% of 10 BOT = **1 BOT** → goes to treasury.
  - Withdrawal Fee = 1% of 110 BOT = **1.1 BOT** → goes to treasury.
  - User receives = **107.9 BOT**.
  - Developer receives = **2.1 BOT**.
...
### 10. Contract Address (Mainnet)

| Parameter          | Value                                      |
|--------------------|--------------------------------------------|
| **Contract Name**  | `AutoYieldVault`                           |
| **Solidity Version** | `0.8.20`                                 |
| **Mainnet Address** | `0x46A38756F329df1bE953e347b4782B3F107ACd6c` |
...
├── lib/
│   ├── chain-config.ts      # Network configuration (mainnet)
...
VITE_VAULT_YIELDS_AGGREGATOR=0x46A38756F329df1bE953e347b4782B3F107ACd6c
VITE_VAULT_STABLE_LP_HUNTER=0x5adD1652646ee604eccFa24c9e2Ab25dB1505066
VITE_VAULT_DELTA_NEUTRAL=0x7a12E3F2fc9603F96C40f4491879AFFd38DEFE23
```
...
- **BOT Scan:** https://scan.botchain.ai
